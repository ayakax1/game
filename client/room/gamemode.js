...
const { room, Game, Players, Inventory, LeaderBoard, BuildBlocksSet, Teams, Damage, BreackGraph, Ui, Properties, GameMode, Spawns, Timers, TeamsBalancer, NewGame, MapEditor, Analytics, Map, Chat } = room_lib;
...
// [PC2] настройки команды чата /skip
const SKIP_COMMAND = "/skip";
const SKIP_REMAINING_TIME = 20; // сколько секунд оставить от основного боя после пропуска
let skipVotes = new Set(); // Id игроков (постоянный player.Id), проголосовавших /skip в текущем основном бою
...
// [PC2] команда чата "/skip" — пропуск оставшегося времени основного боя,
// если ВСЕ находящиеся на сервере игроки написали её
Chat.OnMessage.Add(function (msg) {
	if (msg.IsServerMsg()) return; // серверные сообщения игнорируем
	if (stateProp.Value !== GameStateValue) return; // работает только во время основного боя
	if (!msg.Text || msg.Text.trim().toLowerCase() !== SKIP_COMMAND) return;

	const player = Players.GetByRoomId(msg.Sender);
	if (!player) return;
	if (skipVotes.has(player.Id)) return; // уже голосовал

	skipVotes.add(player.Id);

	const onlinePlayers = Players.All.filter(function (p) { return p.IsOnline; });
	const totalOnline = onlinePlayers.length;
	const votesCount = skipVotes.size;

	if (votesCount >= totalOnline && totalOnline > 0) {
		// все проголосовали — оставляем SKIP_REMAINING_TIME секунд основного боя
		room.PopUp("Все игроки согласились пропустить время! Осталось " + SKIP_REMAINING_TIME + " секунд.");
		mainTimer.Restart(SKIP_REMAINING_TIME);
		skipVotes.clear();
	} else {
		room.PopUp(player.NickName + " предлагает пропустить время (" + votesCount + "/" + totalOnline + "). Напишите " + SKIP_COMMAND + ", чтобы согласиться.");
	}
});
...
function SetGameMode() {
	...
	// [PC2] сбрасываем голоса /skip при входе в новый основной бой
	skipVotes.clear();
