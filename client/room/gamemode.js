...
// [PC2] ручной респавн после суицида (см. ниже) — свой таймер и задержка
const suicideRespawnTimerName = "suicideRespawn";
const SUICIDE_RESPAWN_DELAY = 5; // секунд
const KILLS_PROP_NAME = "Kills";
const ASSISTS_PROP_NAME = "Assists";
const SCORES_PROP_NAME = "Scores";
// [PC2] "KD" — не встроенное свойство движка, доступ строго через Properties.Get()
const KD_PROP_NAME = "KD";

// [PC2] отчёт о матче для сайта — см. buildMatchReportString() в конце файла
let currentMatchId = "";
...
// [PC2] пункт 1: полностью убираем авто-баланс — игрок может зайти в любую
// команду, даже если там уже больше игроков, чем в другой
...
// [PC2] колонки "P" (Scores) и "S" (Spawns) убраны из отображения; вместо них
// добавлена "KD" (Kills/Deaths, ассисты не учитываются). Сама механика очков
// (Scores) при этом никуда не делась — она по-прежнему считает победителя
// матча и работает в овертайме/оповещении, просто больше не показывается
// отдельной колонкой в личной статистике.
LeaderBoard.PlayerLeaderBoardValues = [
	...
	new DisplayValueHeader(KD_PROP_NAME, "Statistics/KD", "Statistics/KDShort")
];
...
Teams.OnPlayerChangeTeam.Add(function (player) {
	// [PC2] явно инициализируем Assists и KD нулём — иначе колонки в лидерборде
	// остаются пустыми у игроков, которые ещё ничего не сделали.
	// Оба имени — не встроенные свойства движка, поэтому доступ строго
	// через Properties.Get(), а не напрямую (.Assists / .KD)
	var assistsProp = player.Properties.Get(ASSISTS_PROP_NAME);
	assistsProp.Value = assistsProp.Value || 0;
	var kdProp = player.Properties.Get(KD_PROP_NAME);
	kdProp.Value = kdProp.Value || 0;
	player.Spawns.Spawn();
});
...
Timers.OnPlayerTimer.Add(function (timer) {
	...
	if (timer.Id == suicideRespawnTimerName) {
		// [PC2] ручной респавн через 5 сек после суицида (см. OnKillReport ниже).
		// Не спавним, если матч уже не в активной фазе (например, время истекло
		// как раз в эти 5 секунд ожидания)
		const s = stateProp.Value;
		if (s === WaitingStateValue || s === MockModeStateValue || s === EndOfMatchStateValue) return;
		Spawns.GetContext(timer.Player).RespawnEnable = true; // возвращаем обычный респавн на будущее
		timer.Player.Spawns.Spawn();
	}
});
...
Damage.OnDeath.Add(function (player) {
	...
	updateKD(player); // [PC2] пересчёт K/D жертвы
});
...
Damage.OnKillReport.Add(function (victim, killer, report) {
	...
	// [PC2] принудительный деспавн — убирает тело жертвы с карты сразу после
	// смерти, чтобы его нельзя было "добить" второй раз до респавна.
	// Специально вызывается отсюда (а не из OnDeath) — при суициде/падении
	// вызов деспавна именно из OnDeath приводил к рассинхрону визуала
	// (труп-манекен на месте смерти при живом и невидимом игроке где-то ещё)
	if (victim) Spawns.GetContext(victim).Despawn();

	damageScores.applyKillReportScores(victim, killer, report);

	if (killer && victim && killer.Team != null && victim.Team != null) {
		if (killer.Team != victim.Team) {
			// [PC2] пункт 3: если убийца из другой команды — засчитываем килл его команде
			addTeamKill(killer.Team);
			updateKD(killer); // [PC2] пересчёт K/D убийцы (только реальный килл, не суицид)
		} else if (killer === victim) {
			// [PC2] суицид (например, подрыв собственной гранатой) — +1 в счёт
			// СВЕРХУ ЭКРАНА команде-противнику. Статистика самого игрока (Kills/
			// Scores/Deaths) при этом не трогается — только командный счётчик.
			const opponentTeam = (victim.Team === blueTeam) ? redTeam : blueTeam;
			addTeamKill(opponentTeam);

			// [PC2] попытка фикса бага с манекеном: при суициде отключаем
			// обычный автоматический респавн этого игрока и через 5 секунд
			// спавним его вручную сами, ровно один раз.
			// ВАЖНО: по документации RespawnEnable "реализовано только для
			// контекста комнаты" — то есть для контекста ИГРОКА эта настройка
			// может не сработать. Если манекен всё ещё появляется — это,
			// скорее всего, причина.
			Spawns.GetContext(victim).RespawnEnable = false;
			victim.Timers.Get(suicideRespawnTimerName).Restart(SUICIDE_RESPAWN_DELAY);
		}
	}
...
function SetGameMode() {
	...
	// [PC2] генерируем ID матча один раз, в момент начала основного боя —
	// используется в отчёте о матче для сайта (см. buildMatchReportString)
	currentMatchId = Date.now().toString(36).toUpperCase();
...
function SetEndOfMatch_EndMode() {
	...
	// на рестарт — это критичный путь, он не должен зависеть ни от чего другого.
	Game.GameOver(finalTeamsBoard);
	mainTimer.Restart(EndOfMatchTime);

	// [PC2] пункт 2: "Команда победила! MVP: ник" — отдельным попапом всей
	// комнате. Встроенный текст "Вы победили!/Вы проиграли!" генерируется
	// движком внутри Game.GameOver() и переписать его нельзя (в API нет для
	// этого метода) — поэтому наш текст идёт ДОПОЛНИТЕЛЬНО, не вместо него.
	// Обёрнуто в try/catch — ошибка тут не должна ничего ломать.
	try {
		room.PopUp(buildEndOfMatchSummary());
	} catch (e) {
		// намеренно проглатываем — рестарт важнее
	}

	// [PC2] отправляем итоги матча через Analytics.LogEvent.
...
// [PC2] "<Команда> победили! MVP: <ник>" (+ EVP вторым игроком матча)
// Победитель для ЭТОЙ строки определяется по счётчику киллов команды
// (тому самому, что виден сверху экрана), а не по очкам Scores — именно
// так это было явно запрошено.
// Рейтинг игроков — среди вообще всех игроков (не по командам), по формуле
// (килы + ассисты) - смерти, чем выше, тем лучше. При равенстве —
// побеждает тот, у кого больше киллов.
// MVP — игрок с лучшим результатом. EVP — второй по этому же рейтингу,
// но только если его результат положительный (килы+ассисты больше смертей)
// и строго меньше, чем у MVP; если такого нет — EVP не показывается.
function buildEndOfMatchSummary() {
	const redKills = redTeam.Properties.Get(KILLS_PROP_NAME).Value || 0;
	const blueKills = blueTeam.Properties.Get(KILLS_PROP_NAME).Value || 0;

	let winnerText;
	if (redKills > blueKills) winnerText = "Красные победили!";
	else if (blueKills > redKills) winnerText = "Синие победили!";
	else winnerText = "Ничья!";

	const ranked = [];
	for (const player of Players.All) {
		const kills = player.Properties.Kills.Value || 0;
		const assists = player.Properties.Get(ASSISTS_PROP_NAME).Value || 0;
		const deaths = player.Properties.Deaths.Value || 0;
		ranked.push({ player: player, kills: kills, combined: kills + assists - deaths });
	}
	...
	let text = winnerText;
	if (ranked.length > 0) {
		text += " MVP: " + ranked[0].player.NickName;
	}
	if (ranked.length > 1 && ranked[1].combined > 0 && ranked[1].combined < ranked[0].combined) {
		text += "\nEVP: " + ranked[1].player.NickName;
	}

	// [PC2] отчёт для бота — судья копирует и присылает боту @PC2Tournaments_bot,
	// он разберёт строку и обновит статистику на сайте (см. match-report-spec)
	try {
		text += "\n\nОтчёт для бота (скопируйте целиком):\n" + buildMatchReportString();
	} catch (e) {
		// если сборка отчёта вдруг упадёт — матч всё равно должен нормально завершиться
	}

	return text;
}

// [PC2] экранирует служебные символы протокола отчёта в произвольном тексте
function pc2EscapeField(str) {
	return String(str == null ? "" : str).replace(/[|~,]/g, "_");
}

// [PC2] позиционно-взвешенная чек-сумма (ловит перестановки символов при
// ручном копировании судьёй, не только опечатки) — mod 65536, в HEX.
// Тот же алгоритм должен быть и на стороне бота (Worker), иначе отчёт
// всегда будет "не проходить" проверку.
function pc2Checksum(str) {
	let sum = 0;
...
// [PC2] собирает строку-отчёт для передачи на сайт через бота.
// Формат: PCL1|matchId|mode|teamA_score|teamB_score|players|checksum
// players: "ник~килы~смерти~ассисты,ник~килы~смерти~ассисты,..."
function buildMatchReportString() {
	const blueScore = blueTeam.Properties.Get(KILLS_PROP_NAME).Value || 0;
	const redScore = redTeam.Properties.Get(KILLS_PROP_NAME).Value || 0;

	const playersField = Players.All.map(function (p) {
		const nick = pc2EscapeField(p.NickName);
		const kills = p.Properties.Kills.Value || 0;
		const deaths = p.Properties.Deaths.Value || 0;
		const assists = p.Properties.Get(ASSISTS_PROP_NAME).Value || 0;
		return nick + "~" + kills + "~" + deaths + "~" + assists;
	}).join(",");
	...
	const body = [
		...
		currentMatchId || Date.now().toString(36).toUpperCase(),
		"TDM",
		blueTeam.Id + "_" + blueScore,
		redTeam.Id + "_" + redScore,
		playersField
	].join("|");

	return body + "|" + pc2Checksum(body);
}
...
// [PC2] пересчитывает K/D игрока (килы делить на смерти, ассисты не учитываются)
// и кладёт результат, округлённый до 1 знака после запятой, в свойство "KD"
// для отображения в лидерборде. Если смертей ещё нет — KD равен числу килов.
function updateKD(player) {
	if (!player) return;
	const kills = player.Properties.Kills.Value || 0;
	const deaths = player.Properties.Deaths.Value || 0;
	const kd = deaths > 0 ? (kills / deaths) : kills;
	const kdProp = player.Properties.Get(KD_PROP_NAME);
	kdProp.Value = Math.round(kd * 10) / 10;
}
