...
// [PC2] пункт 2: "<Команда> победили! MVP: <ник>"
...
// MVP матча — среди вообще всех игроков (не по командам), по формуле
...
function buildEndOfMatchSummary() {
	...
	for (const player of Players.All) {
		...
		const combined = kills + assists - deaths;
		if (mvp === null || combined > mvpScore || (combined === mvpScore && kills > (mvp.Properties.Kills.Value || 0))) {
			mvpScore = combined;
			mvp = player;
		}
	}

	return mvp ? (winnerText + " MVP: " + mvp.NickName) : winnerText;
}
