...
// [PC2] "Assists" — не встроенное свойство движка (в отличие от Kills/Deaths/Scores),
// поэтому доступ строго через Properties.Get(), а не через .Assists напрямую
...
export function applyKillReportScores(victim, killer, report) {
	...
	for (const i of (report.Items || [])) {
		...
		// [PC2] счётчик ассистов для лидерборда (колонка "A")
		var assistsProp = i.Attacker.Properties.Get(ASSISTS_PROP_NAME);
		++assistsProp.Value;
